...
class DeepSeekRouter:
        ...
        with sr.Microphone() as source:
            ...
            while attempts < max_attempts:
                try:
                    ...
                    await self.speak(
                        "Sorry, I couldn't understand that, please repeat.") if attempts != 1 else await self.speak(
                        "Goodbye, Alex")
                    ...
                    recognizer.pause_threshold = float('inf')
                    await self.speak(
                        "Sorry, the speech recognition service is unavailable.") if attempts != 1 else await self.speak(
                        "Goodbye, Alex")
                    recognizer.pause_threshold = 1
                except Exception:
                    ...
                    recognizer.pause_threshold = float('inf')
                    await self.speak(
                        "Sorry, there was an error, please try again.") if attempts != 1 else await self.speak(
                        "Goodbye, Alex")
                    recognizer.pause_threshold = 1
    ...
    async def begin_ai_integration(self):
        try:
            ...
            return Status(code=400, name_fail="SpeechRecognition Listener Exception",
                          description_fail=f"Failure during listening process due to Exception {str(ex)}")
        ...
        try:
            ...
            return Status(code=400, name_fail="AI Generation Exception",
                          description_fail=f"Failure to generate AI response due to error {str(ex)}")
